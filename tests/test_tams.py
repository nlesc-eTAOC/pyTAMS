...
def test_doublewell_2_workers_tams():
    ...
    tams = TAMS(fmodel_t=fmodel, a_args=[])
    transition_proba = tams.compute_probability()
    assert transition_proba == 0.6254745180342447
    Path("input.toml").unlink(missing_ok=True)
...
@pytest.mark.dependency(depends=["test_doublewell_slow_tams"])
def test_doublewell_slow_restore_tams():
