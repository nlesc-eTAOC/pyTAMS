...
class MOCModel(ForwardModel):
    ...
    def system(self, state, dw, dt, forcing=0):
        ...
        self.VS_t, self.VS_ts, self.VS_n, self.VS_s, self.D = np.array([self.VS_t, self.VS_ts, self.VS_n, self.VS_s, self.D]) \
                                                              + deltas[:5] * dt
        self.VS_n -= dw
        self.VS_s += dw
        ...
        if self.temp:
            ...
            self.OHC_t, self.OHC_ts, self.OHC_n, self.OHC_s, self.OHC_d = np.array([self.OHC_t, self.OHC_ts, self.OHC_n, self.OHC_s, self.OHC_d]) \
                                                                          + deltas[5:] * dt
    ...
    # def run_with_forcing(self, N, start_idx, init_state, idx_forcing, Tmax=100, stop_transi=True, stop_on=False, stop_off=False):
    #     """
    #     N is the number of independent trajectories to compute. 
    #     T is the length of the trajectories in years. 
    #     init is the initialization state. Defaults to the ON-state. 
    ...
    #     # Initialize the trajectories, the noise is computed in advance
    #     dW = self.noise * self.E_A * self.S_0 * self.rng.normal(scale=np.sqrt(self.delta_t * (100*365*86400)), size=(t_end-1,N))
    ...
    #             self.VS_mem[idx_traj,0], self.VS_mem[idx_traj,1], self.VS_mem[idx_traj,2], self.VS_mem[idx_traj,3], self.VS_mem[idx_traj,4] = self.VS_t, self.VS_ts, self.VS_n, self.VS_s, self.VS_d
