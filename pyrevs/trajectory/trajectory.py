...
class Trajectory(Generic[T_Noise, T_State]):
    ...
    def __init__(
        ...
        if frozen or fmodel_t is None:
            ...
            if self._traj_cfg.use_noise_history:
                self.set_noise_backlog(self._fmodel.make_noise_history())
    ...
    def advance(
        ...
        The trajectory is advanced until one of the following:

        * the end time is reached (if provided)
        * the maximum number of steps is reached (if provided)
        * the walltime limit is reached
    ...
    def set_noise_backlog(self, backlog: list[T_Noise]) -> None:
        """Set the noise backlog.

        For small models, it might be more efficient to generate
        once the full noise history instead of calling the RNG
        once per step.

        If the list is shorter than the trajectory length, the
        remaining noise increment will be generated by the usual
        fmodel.make_noise() function.

        Note that if the noise is generated from a Numpy RNG,
        use .tolist() to convert it to a list.

        Args:
            backlog: the noise backlog, as a list
        """
        self.noise_backlog = backlog
